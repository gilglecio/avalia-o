...
class Sending extends Model
{
	...
	public function send_mail_bcc()
	{
		...
		foreach ($mail_bcc as $email => $name) {
			...
			$send = $mail->send();
			if (isset($send['error'])) array_push($fails, $input['to']['email']);
		}
	...
	public function after_save()
	{
		...
		if (isset($send_mail_evaluators['error'])) $fails[] = $send_mail_evaluators['error'];
		if (isset($send_mail_bcc['error'])) $fails[] = $send_mail_bcc['error'];
		if (isset($send_email_valued_corrected['error'])) $fails[] = $send_email_valued_corrected['error'];
